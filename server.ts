...
async function handleAnalyzeSite(req: express.Request, res: express.Response) {
  try {
    ...
    let municipality = '', countyName = '', stateName = '', roadName = '', postalCode = '';
    ...
    // Step 1 of the authoritative geological evidence engine:
    // confirm which PIG-PIB map services are reachable and expose their real
    // layer metadata. This is intentionally a coverage check, not a fabricated
    // site observation. Feature-level GetFeatureInfo/WFS extraction comes next.
    if (countryCode === 'PL') {
    ...
    const titles = {
      ...
      soil_and_ground: isPl ? '1. Budowa Geologiczna, Litologia i Warunki Gruntowe' : isDe ? '1. Geologischer Aufbau & Baugrundverhältnisse' : isFr ? '1. Géologie et Mécanique des Sols' : '1. Geological Structure & Soil Mechanics',
      geohazard_risk: isPl ? '2. Geozagrożenia, Sejsmika i Stateczność Skarp' : isDe ? '2. Georisiken & Hangstabilität' : isFr ? '2. Géorisques et Stabilité' : '2. Geohazard & Slope Stability',
      flooding_risk: isPl ? '3. Hydrogeologia, Zwierciadło Wód i Ryzyko Powodziowe' : isDe ? '3. Hydrogeologie & Hochwasserschutz' : isFr ? '3. Hydrogéologie et Risque Inondation' : '3. Hydrological & Flood Inundation Risk',
      zoning_and_land_use: isPl ? '4. Kataster, MPZP i Parametry Urbanistyczne' : isDe ? '4. Kataster, B-Plan & Bauplanungsrecht' : isFr ? '4. Cadastre, PLU et Droits à Bâtir' : '4. Cadastre, Zoning & Planning Parameters',
      building_regulations: isPl ? '5. Odległości od Granic i Wymogi Budowlane' : isDe ? '5. Gesetzliche Abstandsflächen' : isFr ? '5. Règles de Retrait et Distances Séparatives' : '5. Statutory Setbacks & Building Regulations',
      environmental_factors: isPl ? '6. Uwarunkowania Środowiskowe i Obszary Chronione' : isDe ? '6. Umweltschutz & Schutzgebiete' : isFr ? '6. Environnement et Espaces Protégés' : '6. Environmental Overlays & Protected Areas',
      infrastructure_and_access: isPl ? '7. Dostęp do Drogi Publicznej i Infrastruktura Mediów' : isDe ? '7. Erschließung & Versorgungsnetze' : isFr ? '7. Voirie et Réseaux Divers (VRD)' : '7. Infrastructure & Utility Networks',
      market_and_comparables: isPl ? '8. Analiza Rynkowa i Wycena Statystyczna' : isDe ? '8. Marktwertermittlung & Preisindikatoren' : isFr ? '8. Analyse de Marché et Estimation' : '8. Market Analysis & Valuation Model',
    ...
    const reportData = {
      ...
      geohazard_risk: { summary: isPl ? `Nachylenie powierzchni działki wynosi ${evidenceReport.terrain.averageSlopeDegrees}° (${evidenceReport.terrain.slopeCategory}, deniwelacja w obrysie: ${evidenceReport.terrain.elevationDifferenceM} m). Ryzyko osuwiskowe klasyfikowane jest jako ${evidenceReport.terrain.geohazards.landslideSusceptibility.level}.` : `Terrain slope gradient is ${evidenceReport.terrain.averageSlopeDegrees}° (${evidenceReport.terrain.slopeCategory}, elevation relief across plot: ${evidenceReport.terrain.elevationDifferenceM} m). Mass movement and landslide susceptibility is assessed as ${evidenceReport.terrain.geohazards.landslideSusceptibility.level}.`, detail: evidenceReport.terrain.geohazards.landslideSusceptibility.description },
      ...
      zoning_and_land_use: { summary: isPl ? `Teren podlega jurysdykcji planistycznej (${cProfile.planningInstrumentName}). Wiążące przeznaczenie wymaga oficjalnego potwierdzenia.` : `Subject to municipal planning instruments (${cProfile.planningInstrumentName}). Binding land use designations require an official municipal extract.`, detail: evidenceReport.planning.limitation },
      ...
      environmental_factors: { summary: isPl ? `Analiza form ochrony przyrody i otoczenia krajobrazowego.` : `Environmental overlay analysis completed.`, detail: evidenceReport.environment.limitation },
      infrastructure_and_access: { summary: isPl ? `Droga dojazdowa (${evidenceReport.infrastructure.roadAccess.nearestRoadName || evidenceReport.infrastructure.roadAccess.nearestRoadType}) znajduje się w odległości ok. ${evidenceReport.infrastructure.roadAccess.estimatedDistanceM} m.` : `Road access (${evidenceReport.infrastructure.roadAccess.nearestRoadName || evidenceReport.infrastructure.roadAccess.nearestRoadType}) is situated approx. ${evidenceReport.infrastructure.roadAccess.estimatedDistanceM} m from parcel boundary.`, detail: evidenceReport.infrastructure.roadAccess.sourceName },
      market_and_comparables: { summary: isPl ? `Szacunkowa wartość statystyczna gruntu: ${evidenceReport.valuation.indicativeMinPrice.toLocaleString()} – ${evidenceReport.valuation.indicativeMaxPrice.toLocaleString()} ${cProfile.symbol}.` : `Indicative statistical land valuation range: ${evidenceReport.valuation.indicativeMinPrice.toLocaleString()} – ${evidenceReport.valuation.indicativeMaxPrice.toLocaleString()} ${cProfile.symbol}.`, detail: evidenceReport.valuation.disclaimer },
      development_cost_outlook: { summary: isPl ? `Wymagane są badania geotechniczne, mapa do celów projektowych, weryfikacja planowania i warunków przyłączenia.` : `Mandatory pre-construction due diligence includes geotechnical investigation, topographical survey, planning certificate and utility conditions.`, detail: evidenceReport.verificationChecklist },
      key_risks: isPl ? ['Brak otworów wiertniczych na działce – dokładna nośność i głębokość wód pozostają niezmierzone.', 'Wiążące parametry zabudowy wymagają oficjalnego potwierdzenia.', 'Warunki przyłączenia mediów wymagają potwierdzenia przez gestorów.'] : ['No on-site boreholes – exact soil bearing capacity and water table depth remain unmeasured.', 'Legally binding planning rights require municipal confirmation.', 'Utility hookup conditions require formal verification.'],
      opportunities: isPl ? [`Korzystna topografia terenu (${evidenceReport.terrain.averageSlopeDegrees}°).`, `Bliska dostępność drogi (${evidenceReport.infrastructure.roadAccess.estimatedDistanceM} m).`] : [`Favorable topography (${evidenceReport.terrain.averageSlopeDegrees}° slope).`, `Proximity to public roadway (${evidenceReport.infrastructure.roadAccess.estimatedDistanceM} m).`],
...
app.get('/api/reports/:id', (req, res) => { const rep = reportsStore[req.params.id]; return rep ? res.json(rep) : res.status(404).json({ error: 'Report not found' }); });
app.post('/api/reports', (req, res) => { const report = req.body; if (report && report.id) { reportsStore[report.id] = report; return res.json({ success: true, id: report.id }); } return res.status(400).json({ error: 'Invalid report data' }); });
