...
test('support CTA is localized for report languages', () => {
  const cases = [['hr', /Podržite GroundSurf/], ['pl', /Wesprzyj GroundSurf/], ['es', /Apoyar GroundSurf/], ['sk', /Podporte GroundSurf/], ['no', /Støtt GroundSurf/], ['sv', /Stöd GroundSurf/]] as const;
  for (const [language, expected] of cases) {
    ...
    assert.doesNotMatch(html, /Support GroundSurf|Thank you for support|Every supporter receives/i);
  }
});
...
test('Spanish support widget copy is fully localized', () => {
  ...
  assert.equal(copy.heading, 'Apoyar GroundSurf');
  ...
  assert.doesNotMatch(html, /Support GroundSurf|If this report was useful|Thank you for support|Every supporter receives/i);
});


test('Slovak support widget copy is fully localized', () => {
  const copy = supportLandSurfCopy('sk');
  assert.equal(copy.heading, 'Podporte GroundSurf');
