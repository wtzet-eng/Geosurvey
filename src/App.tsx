...
import { getBrowserLanguage, getLocalizedTitle, getFrontPageI18n } from './utils/i18nTitle';
...
export default function App() {
  ...
  const [languageCode, setLanguageCode] = useState<string>(() => getBrowserLanguage());
  ...
  // Handle URL Query Params (Embed & Deep Linking)
  useEffect(() => {
    try {
      ...
      if (params.get('header') === '0' || params.get('noheader') === '1') {
        setHideHeaderInEmbed(true);
      }
      if (params.get('autofit') === '1' || params.get('autofit') === 'true' || params.get('compact') === '1') {
        setIsAutoFitMode(true);
      }
      const modeParam = params.get('mode');
      if (modeParam === 'circle' || modeParam === 'rectangle' || modeParam === 'polygon') {
        setMode(modeParam);
      }
      ...
      if (lCode) {
        const foundLang = REPORT_LANGUAGES.find((l) => l.code.toLowerCase() === lCode.toLowerCase());
        if (foundLang) {
          setLanguageCode(foundLang.code);
        }
      }
      const areaParam = params.get('area');
      if (areaParam && !isNaN(Number(areaParam))) {
        setAreaSize(Math.max(50, Number(areaParam)));
      }
  ...
  // Send postMessage to parent window for iframe auto-resizing in any host website
  useEffect(() => {
    ...
    const postHeightToParent = () => {
      try {
        ...
        window.parent.postMessage(
          {
            type: 'geosurvey:resize',
            height: docHeight,
            reportActive: Boolean(activeReport)
          },
          '*'
        );
      } catch (e) {}
  ...
  // Load saved reports from localStorage and API on mount
  useEffect(() => {
    try {
  ...
  // Sync saved reports to localStorage
  ...
  // When country changes, update default language and reset shape to auto-focus on new country
  const handleCountryChange = (newCode: string) => {
    setCountryCode(newCode);
    setShape(null);
    const countryObj = EUROPEAN_COUNTRIES.find((c) => c.code === newCode);
    if (countryObj) {
      setLanguageCode(countryObj.language);
    }
  };

  // Handle boundary shape change
  const handleShapeChange = (newShape: BoundaryShape | null) => {
    setShape(newShape);
    if (newShape) {
      const calculatedArea = calculateBoundaryArea(newShape, areaSize);
      if (calculatedArea > 0) {
        setAreaSize(calculatedArea);
      }
    }
  };
  ...
  const isBoundaryComplete = Boolean(
    shape &&
      (shape.type === 'circle'
        ? Boolean(shape.center)
        : shape.type === 'rectangle'
        ? (shape.corners?.length || 0) >= 2
  ...
  // If a report is currently active, render the ReportView
  if (activeReport) {
    return (
      <ReportView
        report={activeReport}
    ...
    <div className="min-h-screen bg-slate-50/60 font-sans text-slate-900 pb-16">
      ...
      <main className="mx-auto max-w-6xl px-4 space-y-8 py-6 sm:py-10">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.6fr_1fr]">
          {/* Left Column: Interactive Map & Boundary Tools */}
              ...
              <div className="flex bg-white p-1 rounded-xl border border-slate-200 shadow-2xs">
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-semibold transition ${
                    mode === 'polygon' ? 'bg-slate-900 text-white shadow-2xs' : 'text-slate-600 hover:text-slate-900'
                  }`}
                >
                  <Pentagon className="h-3.5 w-3.5" />
                  <span>{fp.modePoly}</span>
                </button>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-semibold transition ${
                    mode === 'rectangle' ? 'bg-slate-900 text-white shadow-2xs' : 'text-slate-600 hover:text-slate-900'
                  }`}
                >
                  <Square className="h-3.5 w-3.5" />
                  <span>{fp.modeRect}</span>
                </button>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-semibold transition ${
                    mode === 'circle' ? 'bg-slate-900 text-white shadow-2xs' : 'text-slate-600 hover:text-slate-900'
                  }`}
                >
                  <Circle className="h-3.5 w-3.5" />
                  <span>{fp.modeCircle}</span>
                </button>
              </div>
            </div>
            ...
            <div className="text-xs text-slate-500 bg-white p-3 rounded-xl border border-slate-200/80 flex items-center justify-between gap-2">
              {isBoundaryComplete ? (
                <div className="flex items-center gap-1.5 text-slate-900 font-medium">
                  ...
                  <span>
                    Boundary set · approx <strong className="text-primary font-bold">{Math.round(areaSize).toLocaleString()} m²</strong>
                  </span>
                  {shape?.type === 'circle' && <span className="text-slate-400 text-[11px]">(adjust area input to resize)</span>}
                </div>
              ) : (
                <span className="text-slate-500">
                  {mode === 'circle' && 'Click the map to place the circle center.'}
                  {mode === 'rectangle' && 'Click two opposite corners on the map to draw the rectangle.'}
                  {mode === 'polygon' && 'Click sequential points on the map to draw a custom polygon boundary.'}
                </span>
              )}
            </div>
          </div>

          {/* Right Column: Site Configuration & Submit Panel */}
          <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-5 flex flex-col justify-between">
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-700 flex items-center justify-between">
                  <span>{fp.areaLbl}</span>
                  {shape?.type !== 'circle' && isBoundaryComplete && (
                    <span className="text-[11px] text-slate-400 font-normal">(auto-calculated from boundary)</span>
                  )}
                </label>
                <input
                  type="number"
                  ...
                  disabled={shape?.type !== 'circle' && isBoundaryComplete}
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary disabled:opacity-75 transition"
                />
              </div>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-700 flex items-center gap-1">
                  ...
                  <span>{fp.countryLbl}</span>
                </label>
                <select
                  value={countryCode}
                  onChange={(e) => handleCountryChange(e.target.value)}
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition"
                >
              ...
              {errorMessage && (
                <div className="p-3 bg-red-50 text-red-700 rounded-xl text-xs flex items-center gap-2 border border-red-200/60">
            ...
            <div className="space-y-3 pt-2">
              <button
                ...
                disabled={isAnalyzing || !isBoundaryComplete}
                className="w-full py-3.5 px-4 bg-slate-900 hover:bg-slate-800 disabled:bg-slate-200 disabled:text-slate-400 text-white rounded-2xl font-bold text-sm flex items-center justify-center gap-2 shadow-sm transition"
              >
                {isAnalyzing ? (
                  <>
                    <Loader2 className="h-4 w-4 animate-spin" />
                    <span>Gathering governmental data…</span>
                  </>
                ) : (
                  <>
              ...
              <p className="text-center text-[11px] text-slate-400">
                Report will be synthesized in {currentCountry.name}'s official local language options.
              </p>
            </div>
          </div>
        </div>
