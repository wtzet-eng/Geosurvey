...
import { MapPin, Sparkles, Loader2, Circle, Square, Pentagon, Building2, Globe2, Sliders, ChevronRight, AlertCircle } from 'lucide-react';
...
const SUPPORTED_REPORT_LANGUAGE_CODES = new Set(['en', 'de', 'pl', 'nl', 'cs', 'sv', 'sk']);
...
const uiText = (language: string, en: string, nl: string, cs: string, sv: string, sk: string) =>
  language === 'nl' ? nl : language === 'cs' ? cs : language === 'sv' ? sv : language === 'sk' ? sk : en;
...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      if (repId) fetch(`/api/reports/${repId}`).then((res) => (res.ok ? res.json() : null)).then((rep) => { if (rep) setActiveReport(rep); }).catch(() => {});
    } catch (e) { console.warn('Error reading URL search params:', e); }
  }, []);
  ...
  const handleAnalyzeSite = async () => {
    ...
    if (!isBoundaryComplete || !shape) {
      setErrorMessage(uiText(languageCode, 'Please draw a site boundary on the map first.', 'Teken eerst de grens van het perceel op de kaart.', 'Nejprve zakreslete hranici pozemku do mapy.', 'Rita först in tomtgränsen på kartan.', 'Najprv zakreslite hranicu pozemku na mape.'));
      return;
    }
    setIsAnalyzing(true);
    try {
      ...
      if (!res.ok) throw new Error(uiText(languageCode, 'Failed to analyze site. Please try again.', 'De locatieanalyse kon niet worden voltooid. Probeer het opnieuw.', 'Analýzu lokality se nepodařilo dokončit. Zkuste to znovu.', 'Platsanalysen kunde inte slutföras. Försök igen.', 'Analýzu lokality sa nepodarilo dokončiť. Skúste to znova.'));
      ...
      const newReport: SiteReport = reportPayload?.report_data ? {
        ...
        country: currentCountry.name, country_code: currentCountry.code, language: languageCode,
      ...
      setErrorMessage(err.message || uiText(languageCode, 'An error occurred while generating the report.', 'Er is een fout opgetreden bij het maken van het rapport.', 'Při vytváření reportu došlo k chybě.', 'Ett fel uppstod när rapporten skapades.', 'Pri vytváraní reportu sa vyskytla chyba.'));
        ...
        <div className="grid gap-6 lg:grid-cols-[1.6fr_1fr]">
          <div className="space-y-3">
            ...
            <div className="text-xs text-slate-500 bg-white p-3 rounded-xl border border-slate-200/80 flex items-center justify-between gap-2">
              {isBoundaryComplete ? <div className="flex items-center gap-1.5 text-slate-900 font-medium"><span className="h-2 w-2 rounded-full bg-emerald-500" /><span>{uiText(languageCode, 'Boundary set · approx', 'Grens ingesteld · circa', 'Hranice nastavena · přibližně', 'Gräns angiven · cirka', 'Hranica určená · približne')} <strong className="text-primary font-bold">{Math.round(areaSize).toLocaleString()} m²</strong></span>{shape?.type === 'circle' && <span className="text-slate-400 text-[11px]">{uiText(languageCode, '(adjust area input to resize)', '(pas de oppervlakte aan om de grootte te wijzigen)', '(velikost upravíte změnou plochy)', '(justera arean för att ändra storlek)', '(veľkosť upravíte zmenou plochy)')}</span>}</div> : <span className="text-slate-500">{mode === 'circle' && uiText(languageCode, 'Click the map to place the circle center.', 'Klik op de kaart om het middelpunt van de cirkel te plaatsen.', 'Kliknutím do mapy umístěte střed kruhu.', 'Klicka på kartan för att placera cirkelns centrum.', 'Kliknite na mapu a umiestnite stred kruhu.')}{mode === 'rectangle' && uiText(languageCode, 'Click two opposite corners on the map to draw the rectangle.', 'Klik op twee tegenoverliggende hoeken om de rechthoek te tekenen.', 'Klikněte na dva protilehlé rohy a nakreslete obdélník.', 'Klicka på två motsatta hörn för att rita rektangeln.', 'Kliknite na dva protiľahlé rohy obdĺžnika.')}{mode === 'polygon' && uiText(languageCode, 'Click sequential points on the map to draw a custom polygon boundary.', 'Klik achtereenvolgens op punten om een vrije perceelgrens te tekenen.', 'Postupným klikáním zakreslete vlastní hranici polygonu.', 'Klicka punkt för punkt på kartan för att rita en egen tomtgräns.', 'Postupným klikaním zakreslite hranicu polygónu.')}</span>}
            </div>
          </div>
