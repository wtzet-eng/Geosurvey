...
const aspect = (value: unknown) => {
  ...
  const directions: Record<string, string> = { N: 'Norr', NE: 'Nordost', E: 'Öster', SE: 'Sydost', S: 'Söder', SW: 'Sydväst', W: 'Väster', NW: 'Nordväst' };
    ...
    <header className="sticky top-0 z-30 bg-white/95 backdrop-blur border-b border-slate-200 px-4 py-3 shadow-sm">
      <div className="mx-auto max-w-5xl flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0"><div className="font-bold text-sm truncate">SurveyLand</div><div className="text-[10px] text-slate-500 truncate">Preliminär tomtbedömning · underlag i första hand</div></div>
        </div>
    ...
    <main className="mx-auto max-w-5xl px-4 pt-6 space-y-6">
      ...
      <section className="bg-white rounded-3xl border border-slate-200/80 shadow-xs p-6 sm:p-8 space-y-5">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <Metric label="Sluttningsriktning" value={aspect(tech.aspect_direction)} />
        </div>
      ...
      <section className="rounded-3xl bg-amber-50 border border-amber-200 p-6 sm:p-8">
        ...
        <div className="mt-3 space-y-2">{legal.length ? legal.map((item: string, index: number) => <p key={index} className="text-xs text-amber-900/80 leading-relaxed">{item}</p>) : <p className="text-xs text-amber-900/80">SurveyLand är ett inledande screeningverktyg och ersätter inte myndighetshandlingar, fackundersökningar, juridisk rådgivning eller värdering.</p>}</div>
      </section>
    </main>
  </div>;
};
