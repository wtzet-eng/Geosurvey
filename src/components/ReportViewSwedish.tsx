  ...
  return <div className="min-h-screen bg-slate-100/70 text-slate-900 pb-20">
    <header className="sticky top-0 z-30 bg-white/95 backdrop-blur border-b border-slate-200 px-4 py-3 shadow-sm">
      <div className="mx-auto max-w-5xl flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0"><div className="font-bold text-sm truncate">LandSurf</div><div className="text-[10px] text-slate-500 truncate">Preliminär tomtbedömning · underlag i första hand</div></div>
        </div>
    ...
    <main className="mx-auto max-w-5xl px-4 pt-6 space-y-6">
      ...
      <section className="rounded-3xl bg-amber-50 border border-amber-200 p-6 sm:p-8">
        ...
        <div className="mt-3 space-y-2">{legal.length ? legal.map((item: string, index: number) => <p key={index} className="text-xs text-amber-900/80 leading-relaxed">{item}</p>) : <p className="text-xs text-amber-900/80">LandSurf är ett inledande screeningverktyg och ersätter inte myndighetshandlingar, fackundersökningar, juridisk rådgivning eller värdering.</p>}</div>
      </section>
    </main>
  </div>;
};
