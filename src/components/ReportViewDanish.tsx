...
const status = (value: unknown) => {
  ...
  if (code === 'VERIFIED') return 'Verificeret';
  if (code === 'MODELLED') return 'Modelleret';
...
export const ReportViewDanish: React.FC<Props> = ({ report, onBack }) => {
  ...
  const buyer=buyerSummary({slope:tech.slope_degrees, risks, planningConfirmed, floodConfirmed},'da');
  const opening=decisionIntroCopy('da');
  const findings=findingsCopy('da');
  const hasParcel=Boolean(tech.cadastral_parcel_id && present(tech.cadastral_parcel_id)!=='ikke tilgængelig');
  ...
  return <div className="min-h-screen bg-slate-100/70 text-slate-900 pb-20">
    <header className="sticky top-0 z-30 bg-white/95 backdrop-blur border-b border-slate-200 px-4 py-3 shadow-sm">
      <div className="mx-auto max-w-5xl flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0"><div className="font-bold text-sm truncate">GroundSurf</div><div className="text-[10px] text-slate-500 truncate">Foreløbig grundvurdering · evidens først</div></div>
        </div>
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <button type="button" onClick={() => setIsEmbedOpen(true)} className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-indigo-200 bg-indigo-50 text-indigo-700 text-xs font-semibold"><Code className="h-3.5 w-3.5" />Indlejr</button>
          <button type="button" onClick={copyLink} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-slate-200 bg-white text-slate-700 text-xs font-semibold">{copied ? <Check className="h-3.5 w-3.5 text-emerald-600" /> : <Copy className="h-3.5 w-3.5" />}{copied ? 'Kopieret' : 'Del'}</button>
    ...
    <main className="mx-auto max-w-5xl px-4 pt-6 space-y-6">
      <section className="rounded-3xl bg-slate-950 text-white p-6 sm:p-8 shadow-lg">
        <div className="flex items-center gap-2 text-indigo-300 text-xs font-bold uppercase tracking-widest"><FileText className="h-4 w-4" />1. Sammenfatning</div>
      ...
      {checklist.length>0&&<section className="rounded-3xl border border-slate-200 bg-white p-6 sm:p-8"><h2 className="text-lg font-semibold">{opening.nextTitle}</h2><p className="mt-1 text-sm text-slate-600">{opening.nextLead}</p><ol className="mt-4 space-y-4 list-decimal pl-5">{checklist.slice(0,3).map((item:any,index:number)=><li key={index} className="pl-1 text-sm"><h3 className="font-semibold">{item.topic}</h3><p className="mt-1 text-slate-600 leading-relaxed">{item.reason}</p>{item.recommendedAuthorityOrExpert&&<p className="mt-1 text-xs text-slate-500">Myndighed / fagperson: {item.recommendedAuthorityOrExpert}</p>}</li>)}</ol></section>}

      <section className="bg-white rounded-3xl border border-slate-200/80 shadow-xs p-6 sm:p-8 space-y-5">
        <div className="flex items-center gap-3 border-b border-slate-100 pb-4"><div className="h-10 w-10 rounded-2xl bg-indigo-50 text-indigo-700 flex items-center justify-center"><MapPin className="h-5 w-5" /></div><div><div className="text-[10px] font-bold uppercase tracking-widest text-slate-400">2. Sted og grund</div><h2 className="text-lg font-bold">Placering og matrikel</h2></div></div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <Metric label="Valgt areal" value={`${report.area_size.toLocaleString('da-DK')} m²`} />
          <Metric label="Matrikel" value={present(tech.cadastral_parcel_id)} />
      ...
      <Section number="3" title="Jord- og funderingsforhold" section={data.soil_and_ground} />
      <Section number="4" title="Geologiske risici" section={data.geohazard_risk}>{risks.length > 0 && <div className="grid sm:grid-cols-2 gap-3">{risks.map((item: any, index: number) => <div key={`${item.category}-${index}`} className="rounded-2xl border border-slate-200 p-4"><div className="flex items-start justify-between gap-3"><div className="font-bold text-sm">{item.category}</div><span className="text-[10px] font-bold text-slate-500">{status(item.evidence_level)}</span></div><div className="mt-1 text-sm text-slate-700">{present(item.level)}</div>{item.detail && <div className="mt-2 text-xs text-slate-500">{item.detail}</div>}</div>)}</div>}</Section>
      <Section number="5" title="Oversvømmelse og hydrologi" section={data.flooding_risk} />
      <Section number="6" title="Planlægning og arealanvendelse" section={data.zoning_and_land_use} />
      <Section number="7" title="Bygge- og myndighedskrav" section={data.building_regulations} />
      <Section number="8" title="Miljøforhold" section={data.environmental_factors} />
      <Section number="9" title="Infrastruktur og adgang" section={data.infrastructure_and_access} />
      <Section number="10" title="Jordmarked og værdi" section={data.market_and_comparables} />

      {utilities.length > 0 && <section className="bg-white rounded-3xl border border-slate-200/80 shadow-xs p-6 sm:p-8 space-y-4"><div className="text-[10px] font-bold uppercase tracking-widest text-slate-400">11. Forsyning</div><h2 className="text-lg font-bold">Forsyningsoversigt</h2><div className="grid sm:grid-cols-2 gap-3">{utilities.map((item: any, index: number) => <div key={`${item.utility}-${index}`} className="rounded-2xl border border-slate-200 p-4"><div className="font-bold text-sm">{item.utility}</div><div className="text-xs text-slate-600 mt-1">{item.status}</div><div className="text-[10px] text-slate-400 mt-2">{status(item.evidence_level)}</div></div>)}</div></section>}

      <section className="bg-white rounded-3xl border border-slate-200/80 shadow-xs p-6 sm:p-8 space-y-4">
        <div className="flex items-center gap-3"><ShieldAlert className="h-5 w-5 text-amber-600" /><div><div className="text-[10px] font-bold uppercase tracking-widest text-slate-400">12. Anbefalet kontrol</div><h2 className="text-lg font-bold">Undersøgelser og verifikation</h2></div></div>
        {checklist.map((item: any, index: number) => <div key={`${item.topic}-${index}`} className="rounded-2xl border border-slate-200 p-4"><div className="font-bold text-sm">{item.topic}</div><div className="text-xs text-slate-600 mt-1">{item.reason}</div>{item.recommendedAuthorityOrExpert && <div className="text-[11px] text-slate-400 mt-2">Myndighed / fagperson: {item.recommendedAuthorityOrExpert}</div>}</div>)}
      </section>

      <section className="bg-white rounded-3xl border border-slate-200/80 shadow-xs p-6 sm:p-8 space-y-4">
        <div className="flex items-center gap-3"><Database className="h-5 w-5 text-indigo-600" /><div><div className="text-[10px] font-bold uppercase tracking-widest text-slate-400">13. Evidensregister</div><h2 className="text-lg font-bold">Datagrundlag og kilder</h2></div></div>
        {evidence.map((item: any, index: number) => <details key={`${item.id}-${index}`} className="rounded-2xl border border-slate-200 p-4"><summary className="cursor-pointer list-none"><div className="flex items-start justify-between gap-3"><div><div className="font-bold text-sm">{item.category}</div><div className="text-xs text-slate-500 mt-1">{item.claim}</div></div><span className="text-[10px] font-bold text-slate-500 shrink-0">{status(item.status)}</span></div></summary><div className="mt-3 pt-3 border-t border-slate-100 text-xs text-slate-600 space-y-2"><div><strong>Kilde:</strong> {item.sourceName}</div>{item.spatialRelationship && <div><strong>Rumlig relation:</strong> {item.spatialRelationship}</div>}{item.calculationMethod && <div><strong>Metode:</strong> {item.calculationMethod}</div>}{item.limitation && <div><strong>Begrænsning:</strong> {item.limitation}</div>}</div></details>)}
      </section>

      {sources.length > 0 && <section className="bg-white rounded-3xl border border-slate-200/80 shadow-xs p-6 sm:p-8 space-y-3"><h2 className="text-lg font-bold">Datakilder</h2>{sources.map((source: any, index: number) => <div key={`${source.name}-${index}`} className="text-xs text-slate-600"><strong>{source.name}</strong> · {source.verification_status || status(source.status)}</div>)}</section>}
      {legal.length > 0 && <section className="rounded-3xl bg-slate-900 text-slate-200 p-6 sm:p-8 space-y-3"><h2 className="text-lg font-bold text-white">Begrænsninger og forbehold</h2>{legal.map((item: string, index: number) => <p key={index} className="text-xs leading-relaxed text-slate-300">{item}</p>)}</section>}
    </main>
  </div>;
};