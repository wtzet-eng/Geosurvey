...
export function renderDutchLocalizedReport(canonical: CanonicalReport): any {
  ...
  return {
    ...
    riskMatrix: [
      { category: 'Aardverschuivingen', level: canonical.hazards.landslide.classification ? risk(canonical.hazards.landslide.classification) : unavailable, evidence_level: canonical.hazards.landslide.status, detail: canonical.hazards.landslide.classification ? `Classificatie van gevoeligheid voor aardverschuivingen: ${risk(canonical.hazards.landslide.classification)}.` : reason(canonical.hazards.landslide.reasonCode || 'SOURCE_UNAVAILABLE') },
