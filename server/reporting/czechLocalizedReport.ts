...
export function renderCzechLocalizedReport(canonical: CanonicalReport): any {
  ...
  return {
    ...
    riskMatrix: [
      { category: 'Sesuvy', level: canonical.hazards.landslide.classification ? risk(canonical.hazards.landslide.classification) : unavailable, evidence_level: canonical.hazards.landslide.status, detail: canonical.hazards.landslide.classification ? `Klasifikace náchylnosti k sesuvům: ${risk(canonical.hazards.landslide.classification)}.` : reason(canonical.hazards.landslide.reasonCode || 'SOURCE_UNAVAILABLE') },
