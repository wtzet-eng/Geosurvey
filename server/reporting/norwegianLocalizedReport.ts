...
function localizedEvidenceRecord(record: any): any {
  ...
  return {
    ...
    calculationMethod: record.id.startsWith('country-support-') ? 'Kontroll av tilgjengelige nasjonale integrasjoner.' : 'Kildespesifikk innhenting og normalisering til LandSurfs dokumentasjonsmodell.',
...
export function renderNorwegianLocalizedReport(canonical: CanonicalReport): any {
  ...
  const summaryCore = valuationAvailable
    ? `Denne kunnskapsbaserte vurderingen gjelder en tomt i Norge. Kartlagt grunnkontekst: ${geologyUnit}. ${terrainText} Jord: ${soilTexture || reason(canonical.soil.reasonCode || 'PARAMETER_NOT_PROVIDED')}. Indikativ tomteverdi er ${canonical.valuation.min!.toLocaleString('nb-NO')}–${canonical.valuation.max!.toLocaleString('nb-NO')} ${canonical.valuation.currency}.`
    : `Denne kunnskapsbaserte vurderingen gjelder en tomt i Norge. Kartlagt grunnkontekst: ${geologyUnit}. ${terrainText} Jord: ${soilTexture || reason(canonical.soil.reasonCode || 'PARAMETER_NOT_PROVIDED')}. Automatisk tomteverdi vises ikke fordi LandSurf ennå ikke har en tilstrekkelig dokumentert norsk kilde for tomteverdi.`;
