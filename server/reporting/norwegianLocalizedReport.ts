...
export function renderNorwegianLocalizedReport(canonical: CanonicalReport): any {
  ...
  const summaryCore = valuationAvailable
    ...
    : `Denne kunnskapsbaserte vurderingen gjelder en tomt i Norge. Kartlagt grunnkontekst: ${geologyUnit}. ${terrainText} Jord: ${soilTexture || reason(canonical.soil.reasonCode || 'PARAMETER_NOT_PROVIDED')}. Dokumentasjonsscore: ${canonical.evidenceScore.totalScore}/100. Automatisk tomteverdi vises ikke fordi LandSurf ennå ikke har en tilstrekkelig dokumentert norsk kilde for tomteverdi.`;
