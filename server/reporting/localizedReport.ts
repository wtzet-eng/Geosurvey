...
export function renderLocalizedReport(canonical: CanonicalReport, requestedLanguage: string) {
  ...
  return {
    ...
    technicalNarrative: {
      groundwater_depth_m: canonical.geology.groundwaterRegime?.startsWith('Modelled groundwater depth:') ? canonical.geology.groundwaterRegime.replace('Modelled groundwater depth:', '').trim() : unavailable,
