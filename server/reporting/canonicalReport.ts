...
const modelledValuationSource = (profile: CountryAdapterProfile, report: VerifiedSiteReport): string => {
  ...
  if (dynamic?.sourceName && !/^(Configured valuation|SurveyLand|LandSurf)$/i.test(dynamic.sourceName.trim())) return dynamic.sourceName;
  const baseline = profile.baseValuationPerSqm > 0 ? ` · ${profile.baseValuationPerSqm} ${profile.currency}/m²` : '';
  return `LandSurf · ${profile.valuationDataSource}${baseline}`;
};
...
export function createCanonicalReport(report: VerifiedSiteReport, profile: CountryAdapterProfile): CanonicalReport {
  ...
  const geologyAge = c.nationalGeology && nationalEvidenceUsable ? rawGeologicalAge : null;
  const geologyGenesis = c.nationalGeology && nationalEvidenceUsable ? rawGeneticOrigin : null;
  const hasMappedGeology = Boolean(geologyUnit || geologyLithology || geologyAge || geologyGenesis);
  const geologyStatus: EvidenceLevel = c.nationalGeology && nationalEvidenceUsable ? ((context.evidence_level as EvidenceLevel) || report.soil.status) : 'REQUIRES_VERIFICATION';
  const geologyReason: AvailabilityReason | undefined = countryLocationMismatch
    ? 'AUTHORITATIVE_DATA_REQUIRED'
    : c.nationalGeology ? (hasMappedGeology ? undefined : evidenceReason(report, /pgi-(?:smgp|mgp|mlp|engineering)|bgs|brgm|geolog/i) || 'SOURCE_UNAVAILABLE') : 'NOT_SUPPORTED_FOR_COUNTRY';
  ...
  const slovakiaValuation = locationCountryConfirmed && report.countryCode === 'SK' && areaM2 && areaM2 > 0
    ...
    : null;
  const baseEvidenceRecords = countryLocationMismatch ? report.evidenceRegistry : visibleEvidenceRecords(report, profile, support);
  const rawRecords = baseEvidenceRecords.filter(record => locationCountryConfirmed || !/valuation|market benchmark|price/i.test(`${record.id} ${record.category}`));
  const records = germanyValuation
    ...
    ? rawRecords.map(record => record.id === 'valuation-indicative-model'
      ? {
          ...record,
          claim: `Indicative Slovak land asking-price benchmark: ${slovakiaValuation.totalMin.toLocaleString()}–${slovakiaValuation.totalMax.toLocaleString()} € (~${slovakiaValuation.unitMedianPrice} €/m²), based on ${slovakiaValuation.benchmark.label}.`,
          sourceName: slovakiaValuation.benchmark.sourceName,
          sourceUrl: slovakiaValuation.benchmark.sourceUrl,
          datasetDate: slovakiaValuation.benchmark.datasetDate,
          spatialRelationship: `${slovakiaValuation.benchmark.tier.toUpperCase()} asking benchmark for ${report.parcel.commune || report.parcel.voivodeship || 'Slovakia'}`,
          calculationMethod: `${slovakiaValuation.benchmark.tier} residential/building-plot asking-price benchmark with parcel-size, road-access and terrain screening adjustments; no generic municipality uplift`,
          confidence: 'Low' as const,
          limitation: 'Land-only asking-price screening benchmark. It is based on advertised residential/building-plot prices, not completed cadastral sale prices or direct comparable deeds. Buildings and other improvements are excluded. Binding ÚPN/buildability, servicing, legal road access and parcel-specific comparable evidence remain unverified; a market appraisal requires current local evidence and a qualified valuer.'
        }
      : record)
    : rawRecords;
  const baseSourceRecords = countryLocationMismatch ? report.dataSourcesCited : visibleSourceRecords(report, support);
  const rawSourceRecords = baseSourceRecords.filter(source => locationCountryConfirmed || source.type !== 'Statistical Market Benchmark');
  const sourceRecords = germanyValuation
    ...
    : slovakiaValuation
    ? rawSourceRecords.map(source => source.type === 'Statistical Market Benchmark'
      ? { ...source, name: slovakiaValuation.benchmark.sourceName, organization: 'ZRKS / Slovak residential-plot listing-market sources', url: slovakiaValuation.benchmark.sourceUrl, status: 'MODELLED' as const }
      : source)
    : rawSourceRecords;
