...
export function renderCroatiaLocalizedReport(canonical: CanonicalReport): any {
  ...
  const groundContext = {
    ...
    mapped_units_label:c.mappedUnits,mapped_units:(mapped?.distinctMappedUnits || []).map((value: string) => localScientificValue(value)),material_indicators_label:c.materialIndicators,material_indicators:(mapped?.materialIndicators || []).map((value: string) => localScientificValue(value)),transition_indicated:Boolean(mapped?.transitionIndicated),sample_label:c.mappedSamples,
  ...
  return {
    ...
    technicalNarrative:{groundwater_depth_m:c.unavailable,groundwater_notice:canonical.support.capabilities.nationalHydrogeology?reason('AUTHORITATIVE_DATA_REQUIRED'):reason('NOT_SUPPORTED_FOR_COUNTRY'),zoning_name:canonical.countryCode === 'HR' ? 'Važeća prostorno-planska dokumentacija / ISPU' : canonical.planning.instrumentName,max_far:c.unavailable,max_building_coverage_pct:c.unavailable,min_biologically_active_pct:c.unavailable,max_height_m:c.unavailable,utility_status:reason('AUTHORITATIVE_DATA_REQUIRED')},
    riskMatrix:[
