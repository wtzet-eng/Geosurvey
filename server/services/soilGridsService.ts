...
function estimateBearingParameters(sand: number, silt: number, clay: number, bulkDensity: number): {
  ...
  if (sand >= 65) { frictionAngle = Math.round(31 + (bulkDensity - 1.3) * 12); cohesion = 2; baseBearing = Math.round(180 + (bulkDensity - 1.3) * 120); kSat = '5.0 × 10⁻⁴ m/s (Fast draining)'; drainage = 'High permeability / Good natural drainage'; frost = silt < 10 ? 'F1 (Non-frost-susceptible)' : 'F2 (Low-to-medium frost susceptibility)'; }
  else if (clay >= 35) { frictionAngle = 18; cohesion = Math.round(20 + (bulkDensity - 1.2) * 25); baseBearing = Math.round(140 + (bulkDensity - 1.2) * 100); kSat = '1.0 × 10⁻⁸ m/s (Low permeability)'; drainage = 'Poor natural permeability / Water retention potential'; frost = 'F3 (High frost susceptibility)'; }
  else if (silt >= 45) { frictionAngle = 24; cohesion = 10; baseBearing = Math.round(150 + (bulkDensity - 1.3) * 90); kSat = '2.0 × 10⁻⁶ m/s'; drainage = 'Moderate to slow permeability'; frost = 'F3 (High frost susceptibility)'; }
  else { frictionAngle = 27; cohesion = 12; baseBearing = Math.round(190 + (bulkDensity - 1.3) * 110); kSat = '5.0 × 10⁻⁶ m/s'; drainage = 'Moderate permeability'; frost = silt > 25 ? 'F3 (High frost susceptibility)' : 'F2 (Low-to-medium frost susceptibility)'; }
  const minB = Math.max(120, Math.round(baseBearing * 0.85));
  const maxB = Math.round(baseBearing * 1.25);
  return { bearingCapacityKpa: baseBearing, bearingCapacityStr: `${minB} – ${maxB} kPa`, frictionAngleDeg: frictionAngle, cohesionKpa: cohesion, hydraulicConductivityMs: kSat, drainageClass: drainage, frostSusceptibility: frost };
}
...
export async function fetchGenuineSoilGridsData(lat: number, lng: number): Promise<SoilGridsResult> {
  ...
  const fallback: SoilGridsResult = {
    success: false, sourceName: 'ISRIC - World Soil Information (SoilGrids 2.0 Global Dataset)', sourceUrl: 'https://www.isric.org/explore/soilgrids', datasetVersion: 'SoilGrids v2.0 (250m Resolution)', usdaTextureClass: 'Not available', topsoilClayPct: NaN, topsoilSandPct: NaN, topsoilSiltPct: NaN, subsoilClayPct: NaN, subsoilSandPct: NaN, subsoilSiltPct: NaN, meanBulkDensityGcm3: NaN, meanPhH2O: NaN, meanOrganicCarbonPct: NaN, estimatedBearingCapacityKpa: 'Not available', effectiveFrictionAngleDeg: NaN, cohesionKpa: NaN, hydraulicConductivityMs: 'Not available', drainageClass: 'Not available', frostSusceptibilityClass: 'Not available', topsoilStrippingDepthCm: NaN, stratigraphyProfile: [], limitation: 'SoilGrids query unavailable or invalid. No site soil classification or engineering parameter has been inferred; a Eurocode 7 site investigation is required.'
  };
  try {
    ...
    const res = await fetch(url, { headers: { Accept: 'application/json', 'User-Agent': 'EuropeanLandValuationEngine/5.0 (Research & Geospatial Verification)' }, signal: controller.signal });
    clearTimeout(timer);
    if (!res.ok) return await fetchSoilGridsWmsData(lat, lng, fallback);
    ...
    const sandLayer = findProp('sand'); const siltLayer = findProp('silt'); const clayLayer = findProp('clay'); const socLayer = findProp('soc'); const bdodLayer = findProp('bdod'); const phLayer = findProp('phh2o'); const cecLayer = findProp('cec');
    const getVal = (layer: any, depthLabel: string): number | undefined => { if (!layer || !Array.isArray(layer.depths)) return undefined; const dObj = layer.depths.find((d: any) => d.label === depthLabel); const value = dObj?.values?.mean ?? dObj?.values?.median; return typeof value === 'number' && Number.isFinite(value) ? value : undefined; };
    const stratigraphyProfile: SoilLayerDepth[] = [];
    const depthDefs = [
      { label: '0-5cm', top: 0, bottom: 5 }, { label: '5-15cm', top: 5, bottom: 15 }, { label: '15-30cm', top: 15, bottom: 30 }, { label: '30-60cm', top: 30, bottom: 60 }, { label: '60-100cm', top: 60, bottom: 100 }, { label: '100-200cm', top: 100, bottom: 200 }
    ];
    for (const def of depthDefs) {
      const rawSand = getVal(sandLayer, def.label); const rawSilt = getVal(siltLayer, def.label); const rawClay = getVal(clayLayer, def.label); const rawSoc = getVal(socLayer, def.label); const rawBdod = getVal(bdodLayer, def.label); const rawPh = getVal(phLayer, def.label); const rawCec = getVal(cecLayer, def.label);
      if ([rawSand, rawSilt, rawClay, rawSoc, rawBdod, rawPh, rawCec].some(value => value === undefined) || (rawSand! + rawSilt! + rawClay!) <= 0) return await fetchSoilGridsWmsData(lat, lng, fallback);
      const sandPct = Math.round((rawSand! / 10) * 10) / 10; const siltPct = Math.round((rawSilt! / 10) * 10) / 10; const clayPct = Math.round((rawClay! / 10) * 10) / 10; const socPct = Math.round((rawSoc! / 100) * 100) / 100; const bulkDensity = Math.round((rawBdod! / 100) * 100) / 100; const ph = Math.round((rawPh! / 10) * 10) / 10; const cec = Math.round(rawCec! * 10) / 10;
      const textureClass = getUsdaTextureClass(sandPct, siltPct, clayPct); const params = estimateBearingParameters(sandPct, siltPct, clayPct, bulkDensity);
      stratigraphyProfile.push({ depthRange: `${def.top} – ${def.bottom} cm`, topDepthCm: def.top, bottomDepthCm: def.bottom, sandPct, siltPct, clayPct, bulkDensityGcm3: bulkDensity, soilOrganicCarbonPct: socPct, phH2O: ph, cec, textureClass, estimatedBearingCapacityKpa: params.bearingCapacityKpa, mechanicalDescription: `${textureClass}; modelled pedological profile (not site-specific geotechnical testing)` });
    }
    const topsoilLayers = stratigraphyProfile.filter(l => l.bottomDepthCm <= 30); const subsoilLayers = stratigraphyProfile.filter(l => l.topDepthCm >= 30);
    const avg = (arr: SoilLayerDepth[], key: keyof SoilLayerDepth) => arr.reduce((sum, l) => sum + (l[key] as number), 0) / arr.length;
    const topSand = avg(topsoilLayers, 'sandPct'); const topSilt = avg(topsoilLayers, 'siltPct'); const topClay = avg(topsoilLayers, 'clayPct'); const subSand = avg(subsoilLayers, 'sandPct'); const subSilt = avg(subsoilLayers, 'siltPct'); const subClay = avg(subsoilLayers, 'clayPct'); const meanDensity = avg(stratigraphyProfile, 'bulkDensityGcm3'); const meanPh = avg(stratigraphyProfile, 'phH2O'); const meanSoc = avg(stratigraphyProfile, 'soilOrganicCarbonPct'); const topTexture = getUsdaTextureClass(topSand, topSilt, topClay); const mech = estimateBearingParameters(topSand, topSilt, topClay, meanDensity);
    return { success: true, sourceName: 'ISRIC - World Soil Information (SoilGrids 2.0 REST API)', sourceUrl: url, datasetVersion: 'SoilGrids v2.0 (250m Resolution)', usdaTextureClass: topTexture, topsoilClayPct: Math.round(topClay * 10) / 10, topsoilSandPct: Math.round(topSand * 10) / 10, topsoilSiltPct: Math.round(topSilt * 10) / 10, subsoilClayPct: Math.round(subClay * 10) / 10, subsoilSandPct: Math.round(subSand * 10) / 10, subsoilSiltPct: Math.round(subSilt * 10) / 10, meanBulkDensityGcm3: Math.round(meanDensity * 100) / 100, meanPhH2O: Math.round(meanPh * 10) / 10, meanOrganicCarbonPct: Math.round(meanSoc * 100) / 100, estimatedBearingCapacityKpa: mech.bearingCapacityStr, effectiveFrictionAngleDeg: mech.frictionAngleDeg, cohesionKpa: mech.cohesionKpa, hydraulicConductivityMs: mech.hydraulicConductivityMs, drainageClass: mech.drainageClass, frostSusceptibilityClass: mech.frostSusceptibility, topsoilStrippingDepthCm: 30, stratigraphyProfile, limitation: 'SoilGrids values are modelled 250 m pedological estimates. Engineering parameters derived from texture are preliminary only and require site-specific Eurocode 7 verification.' };
  } catch (err) {
