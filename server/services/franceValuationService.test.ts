...
const land = (unitPrice: number, area = 1000, date = '2025-06-01', code = '21') => ({
  codtypbien: code, libtypbien: 'TERRAIN DE TYPE TAB', libnatmut: 'Vente', vefa: false,
...
test('DVF summary keeps detailed bare-land codes and rejects built/VEFA/non-land records', () => {
  const rows: any[] = [land(80, 1000, '2025-01-01', '20'), land(90, 1000, '2025-02-01', '21'), land(100, 1000, '2025-03-01', '2311'), land(110, 1000, '2025-04-01', '232'), land(120, 1000, '2025-05-01', '239'),
    { ...land(900), codtypbien: '1113' }, { ...land(700), sbati: '120' }, { ...land(600), vefa: true }, { ...land(500), libnatmut: 'Echange' }
  ];
  const summary = summarizeFranceLandTransactions(rows, 'COMMUNE', commune);
  assert.ok(summary); assert.equal(summary?.sampleCount, 5); assert.equal(summary?.benchmarkPricePerSqm, 100); assert.equal(summary?.q25PricePerSqm, 90); assert.equal(summary?.q75PricePerSqm, 110);
});

test('nearby DVF bare-land sample wins when it is sufficient', async () => {
  ...
  assert.equal(evidence.status, 'MODELLED'); assert.equal(evidence.id, 'fr-dvf-land-valuation'); assert.equal((evidence.value as any).scope, 'LOCAL'); assert.equal((evidence.value as any).benchmarkPricePerSqm, 95); assert.equal((evidence.value as any).sampleCount, 6);
});

test('commune DVF sample is used when nearby evidence is too sparse', async () => {
  const local = [80, 90, 100].map(value => land(value));
  const communeWide = [100, 110, 120, 130, 140, 150].map(value => land(value));
  const evidence = await queryFranceLandValuationEvidence(44.84, -0.58, fetcher(local, communeWide));
  assert.equal(evidence.status, 'MODELLED'); assert.equal((evidence.value as any).scope, 'COMMUNE'); assert.equal((evidence.value as any).benchmarkPricePerSqm, 125);
});
...
test('insufficient DVF evidence fails closed and never revives a generic French baseline', async () => {
  ...
  assert.equal(evidence.status, 'REQUIRES_VERIFICATION'); assert.equal(evidence.reasonCode, 'NO_DATA');
  ...
  assert.equal(report.valuation.status, 'REQUIRES_VERIFICATION'); assert.ok(Number.isNaN(report.valuation.indicativeMedianPrice)); assert.doesNotMatch(report.valuation.methodology, /205\s*€?\/m²/i); assert.match(report.valuation.methodology, /No generic national/i);
});
...
test('Alsace-Moselle and Mayotte DVF coverage exclusions fail closed', async () => {
  for (const department of ['57', '67', '68', '976']) {
    ...
    assert.equal(evidence.status, 'REQUIRES_VERIFICATION'); assert.equal(evidence.id, 'fr-dvf-land-valuation-coverage-excluded'); assert.match(evidence.limitation, /Alsace-Moselle and Mayotte/i);
  }
});
...
test('valid DVF benchmark enriches report as land-only value with real comparable count', async () => {
  ...
  enrichFranceValuationFromEvidence(report, [evidence as any]);
  assert.equal(report.valuation.status, 'MODELLED'); assert.equal(report.valuation.indicativePricePerSqm, 125); assert.equal(report.valuation.indicativeMedianPrice, 125000); assert.equal(report.valuation.comparableEvidenceCount, 8);
  assert.match(report.valuation.methodology, /DVF\+/i); assert.match(report.valuation.methodology, /Buildings and other improvements are excluded/i); assert.match(report.valuation.disclaimer, /LAND VALUE ONLY/i); assert.match(report.dataSourcesCited[0].name, /Cerema DVF\+/i); assert.equal(report.evidenceScore.breakdown.planningAndMarket.score, 6);
});
