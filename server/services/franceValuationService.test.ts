...
const land = (unitPrice: number, area = 1000, date = '2025-06-01') => ({
  codtypbien: '2', libtypbien: 'DU FONCIER NU', libnatmut: 'Vente', vefa: false,
...
test('DVF summary keeps bare-land sales and rejects built/VEFA/non-land records', () => {
  const rows: any[] = [land(80), land(90), land(100), land(110), land(120),
    { ...land(900), codtypbien: '1' },
    { ...land(700), sbati: '120' },
    { ...land(600), vefa: true },
...
test('insufficient DVF evidence fails closed and never revives a generic French baseline', async () => {
  ...
  const report: any = {
    valuation: { status: 'MODELLED', indicativeMinPrice: 205000, indicativeMaxPrice: 205000, indicativeMedianPrice: 205000, indicativePricePerSqm: 205, currency: 'EUR' },
    evidenceScore: { breakdown: { planningAndMarket: { score: 4, max: 10, rationale: 'old baseline' } } }, dataSourcesCited: []
  };
...
test('valid DVF benchmark enriches report as land-only value with real comparable count', async () => {
  ...
  const report: any = {
    ...
    valuation: {},
    evidenceScore: { breakdown: { planningAndMarket: { score: 0, max: 10, rationale: '' } } },
    dataSourcesCited: [{ name: 'old', organization: 'old', url: '', type: 'Statistical Market Benchmark', status: 'MODELLED' }]
  };
  enrichFranceValuationFromEvidence(report, [evidence as any]);
  ...
  assert.match(report.valuation.methodology, /DVF\+/i);
