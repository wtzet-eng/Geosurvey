...
const caps='<WFS_Capabilities><FeatureTypeList><FeatureType><Name>mt:test_layer</Name><Title>Test layer</Title></FeatureType></FeatureTypeList></WFS_Capabilities>';
...
test('Malta WFS client falls back from WFS 2 to WFS 1.1 GeoJSON query',async()=>{
  let getFeatureCalls=0;
  const fetcher:typeof fetch=async(input:any)=>{
    const url=new URL(String(input));
    if(url.searchParams.get('REQUEST')==='GetCapabilities') return response(caps,200,'text/xml');
    getFeatureCalls+=1;
    if(url.searchParams.get('VERSION')==='2.0.0') return response('down',503,'text/plain');
    return response(JSON.stringify({type:'FeatureCollection',features:[]}));
  };
  const result=await queryMaltaWfs('https://example.test/wfs',35.9,14.51,20,fetcher);
  assert.equal(result.status,'OK');
  assert.equal(getFeatureCalls,2);
});
