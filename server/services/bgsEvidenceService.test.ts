...
test('missing one BGS field does not discard the other mapped scientific fields', async () => {
  ...
  const result = await fetchBgsSiteEvidence(52, -1, mockFetch(url => url.includes('/detailed/4/query') ? partial : url.includes('/detailed/3/query') ? { features: [] } : emptyContext(url)), endpoints);
